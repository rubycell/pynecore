...
@script.strategy('L3e short stop', overlay=True, pyramiding=0, initial_capital=500000000, default_qty_type=strategy.fixed, default_qty_value=1, margin_long=18.48, margin_short=18.48, calc_on_every_tick=False, process_orders_on_close=False)
def main():
    traded: PersistentSeries[bool] = False
    closing: PersistentSeries[bool] = False

    if barstate.isrealtime and (not traded):
        strategy.entry("E", strategy.short, stop=low[1], comment="STOP short")
        traded = True

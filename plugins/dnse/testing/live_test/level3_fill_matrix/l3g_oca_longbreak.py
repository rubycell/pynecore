...
@script.strategy('L3g OCA long-break', overlay=True, pyramiding=0, initial_capital=500000000, default_qty_type=strategy.fixed, default_qty_value=1, margin_long=18.48, margin_short=18.48, calc_on_every_tick=False, process_orders_on_close=False)
def main():
    traded: PersistentSeries[bool] = False
    closing: PersistentSeries[bool] = False

    if barstate.isrealtime and (not traded):
        strategy.entry("Up", strategy.long, stop=high[1], oca_name="g", oca_type=strategy.oca.cancel, comment="OCA up (fills)")
        strategy.entry("Dn", strategy.short, stop=low[1] * 0.95, oca_name="g", oca_type=strategy.oca.cancel, comment="OCA dn (far, cancels)")
        traded = True
    ...
    if strategy.position_size > 0:
        if na(slLevel):
            slLevel = low[1]
        strategy.exit("X", from_entry="Up", stop=slLevel, comment_loss="SL@" + string.tostring(slLevel, format.mintick))

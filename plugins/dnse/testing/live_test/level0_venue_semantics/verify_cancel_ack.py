...
def venue_status(broker: DNSEBroker, order_id: str) -> str | None:
    status, body = broker.client.get_order_detail(
        broker.account_id, order_id, broker.market_type, order_category="STOP")
...
def main() -> int:
    ...
    try:
        ...
        print(f"\nplaced STOP id={order_id}  filled={orders[0].filled_qty}")
        print(f"  venue status before cancel: {venue_status(broker, order_id)}")

        polls["n"] = 0
        t0 = time.monotonic()
        confirmed = broker._cancel_one(order_id)
        elapsed = time.monotonic() - t0
        after = venue_status(broker, order_id)
        ...
        if order_id:
            ...
            try:
                print("  cancel ->", broker._cancel_one(order_id),
                      "| venue:", venue_status(broker, order_id))

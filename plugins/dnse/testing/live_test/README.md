...
| **Live-L3-F05-LongStopLimit** … **F06-ShortStopLimit** | stop-limit fills (#14 evidence) | ❌ **F05 09-07 CRITICAL #82** (TWO naked-short doors, same root — a protective exit acting with no position): **#82a** marketable-CLOSE-while-flat FIXED 8c02e86 (guard held on re-grade); **#82b** the native protection conditional is dispatched to the venue AT PLACEMENT before the entry fills and fills naked when the market crosses it — OPEN. Placement itself correct (ONE STOP+limit, #14 ✓). Both incidents recovered flat in seconds. F06 not attempted — parked on #82b |

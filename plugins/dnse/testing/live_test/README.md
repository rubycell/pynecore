...
| **Live-L1-T22-ExpiryRouting** | #94 post-fix grade: place a far NORMAL LO ~14:40, HOLD through the 14:45 expiry with the engine live → the venue Expired order must route WITHOUT quarantining the run (engine keeps trading-eligible state; venue record Expired fill=0; prune fires). Pre-fix this quarantines (probe-measured). Close-window gated (14:40-14:50) | ✅ 09-09 PASS live (combo run): LO 244006 placed 14:20, held through ATC; venue flipped `Expired` at ~15:04 (expiry is a deferred batch, +19 min — fact revised); engine's poll ladder observed it, journal closed with `terminal_status: Expired`, ZERO quarantine/unexpected lines, strategy got a clean CANCELLED event. Same run: watchdog half of #98 PASS (no stale-abort/reconnect across the 16-min gap), filler half FAILED → #102 (idle-synth fabricated V=0 bars through the calendar-open ATC; quiet-phase gate nested under the market-closed branch) |
...
- **DAY expiry is a deferred batch, NOT 14:45-sharp** (measured 09-09, T22): an
  ...
  15:05; a run watching for its own expiry event must stay alive that long.

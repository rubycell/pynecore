...
| **Live-L3-F01-LongMarket** … **F02-ShortMarket** | market fills | ❌ NOT RUN (unaffected by #39 — F10 proved the plain-LO fill path works) |
| **Live-L3-F03-LongStop** … **F04-ShortStop** | stop-entry fills | ⛔ BLOCKED BY #39 (stop-entry fills are invisible to the plugin) |
| **Live-L3-F05-LongStopLimit** … **F06-ShortStopLimit** | stop-limit fills (#14 evidence) | ⛔ BLOCKED BY #39 |
| **Live-L3-F07-OcaLongBreak** … **F08-OcaShortBreak** | OCA sibling-cancel on a real fill | ⛔ BLOCKED BY #39 (both use stop-entry legs) |
...
| **Live-L3-F11-OcaEntryGroupCancel** | oca.cancel ENTRY pair: near fills → far leg must be CANCELLED by the engine cascade (`live_oca_entry_group.pine`; oracle measured 08-18: backtest cancels it; fix #33 flipped oca_cancel→SOFTWARE) | ⛔ **08-18 BLOCKED BY #39** — near stop TRIGGERED (Activated, child normal order 437346 filled, account +1) but the plugin never surfaced the fill: strategy stayed pos=0, no cascade, no flatten, operator flattened manually. F11's own question (cascade) UNANSWERED; re-run after #39. Also proves the two-book mechanic now in CLAUDE.md |

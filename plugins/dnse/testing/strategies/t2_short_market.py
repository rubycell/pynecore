...
@script.strategy("T2 Short, market entry on red candle", overlay=True, pyramiding=0,
...
def main():
    ...
    if flat and is_red:
        strategy.entry("S", strategy.short, comment=f"E-mkt@{close}")

    if in_short and close <= strategy.position_avg_price * (1 - TAKE_PCT):
        strategy.close("S", comment=f"TP@{close}")
    elif in_short and is_green:
        strategy.close("S", comment=f"X-green@{close}")

...
@script.strategy("T6 Short, stop+limit entry and bracketed exit", overlay=True, pyramiding=0,
...
def main():
    ...
    if flat:
        strategy.entry("S", strategy.short, stop=low[1], limit=low[1] * 0.999,
                       comment=f"E-sl@{low[1]}")

    if in_short:
        target = strategy.position_avg_price * 0.99
        strategy.exit("X", from_entry="S", stop=high[1], limit=target,

"""DNSE data provider plugin for PyneCore.

Historical OHLCV over ``GET /price/ohlc``. Verified against the live API:

    type=DERIVATIVE symbol=VN30F1M resolution=15  ->  17 bars/trading day
    (09:00-11:30 + 13:00-14:45 ICT), payload {t,o,h,l,c,v,nextTime}

The continuous alias ``VN30F1M`` is accepted directly; dated contract codes
(e.g. VN30F2612) return HTTP 400, so no front-month roll is needed.
"""
...
class DNSEConfig(LiveProviderConfig):
    ...
    :ivar api_version: Value of the ``version`` header.
    ...
    #: WebSocket origin. Configurable for the same reason as ``base_url``:
    #: pointing both at a local fake venue is how the plugin is tested without
    #: DNSE trading hours (VN30F1M is orderable only 09:00-14:45 ICT).
...
class DNSEProvider(ProviderPlugin[DNSEConfig]):
    ...
    def resolve_contract(self, symbol: str | None = None) -> str:
        ...
        dated contract (``41I1G8000``). ``/instruments`` carries both, so the
        alias resolves to the front month here and is cached per instance.
        ...
        status, body = self.client.request("GET", "/instruments",
                                           query={"limit": 200})
        resolved = wanted
    ...
    def normalize_symbol(self, symbol: str) -> str:
        ...
        Called by the live runner before :meth:`watch_ohlcv`. Historical
        methods keep using ``self.symbol`` (the alias), which is what
        ``/price/ohlc`` accepts — the two paths genuinely differ.
        """
    ...
    def _secdef(self, symbol: str) -> dict:
        ...
        status, body = self.client.get_secdef(lookup)
        row = body[0] if status == 200 and isinstance(body, list) and body else {}
        cache[symbol] = row
        return row

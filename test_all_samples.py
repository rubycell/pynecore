#!/usr/bin/env python3
"""
Test all Pine Script samples: transpile to Python, then run with pyne.

Usage:
    python test_all_samples.py                    # Run all samples
    python test_all_samples.py "ex_001*"          # Filter by pattern
    python test_all_samples.py --timeout 30       # Custom timeout
    python test_all_samples.py --verbose          # Show stderr on failure
    python test_all_samples.py --retry-failed     # Only run files that failed last time
    python test_all_samples.py -j 24              # Run on 24 cores in parallel
    python test_all_samples.py -j 0              # Use all available CPU cores
"""
...
SAMPLE_DIR = BASE_DIR / 'sample' / 'pinescript'
...
def classify_error(error_text, verbose=False):
    ...
    # Print full error text when unknown and verbose mode is enabled
    if verbose:
...
def load_failed_files():
    ...
    Returns:
        None: No test results file exists
        'missing_key': Test results exist but don't have failed_files key (old format)
        set: Set of failed filenames (may be empty if all passed)
    """
    json_file = BASE_DIR / 'test_results.json'
    if not json_file.exists():
        return None

    try:
        data = json.loads(json_file.read_text())
        # Support both old array format and new single-object format
        if isinstance(data, list) and len(data) > 0:
            # Old format: array of runs, get the last one
            last_run = data[-1]
        elif isinstance(data, dict):
            # New format: single run object
            last_run = data
        else:
            return None

        # Check if failed_files key exists
...
def test_file(pine_file, pyne_bin, data_file, timeout=15, verbose=False):
    ...
    try:
        ...
        # Skip runtime test for empty output (comment-only files)
        if not output:
            result['transpile_time'] = (datetime.now() - start).total_seconds()
            result['transpile'] = True
            result['runtime'] = True  # Treat empty as runtime success (skip)
            return result
...
def save_reports(results, total, error_counts, missing_imports, missing_apis, skipped=0):
    """Save JSON and text reports, appending to history."""
    transpile_ok = sum(1 for r in results if r['transpile'])
    transpile_fail = total - transpile_ok
    runtime_ok = sum(1 for r in results if r['runtime'])
    runtime_fail = sum(1 for r in results if r['transpile'] and not r['runtime'])

    # Collect failed filenames for retry feature
    ...
    # JSON report - keep only current run + 1 backup
    json_file = BASE_DIR / 'test_results.json'
    backup_file = BASE_DIR / 'test_results.json.bak'
    ...
    # Write new results (single object, not array)
    with json_file.open('w') as f:
        json.dump(run_entry, f, indent=2)

    # Text summary - keep only last 5 runs
    txt_file = BASE_DIR / 'test_results.txt'
    ...
    # Read existing entries and keep last 4
    existing_runs = []
    if txt_file.exists():
        try:
            content = txt_file.read_text()
            # Split by separator (each run starts with \n====)
            runs = content.split('\n' + '=' * 70 + '\n')
            # Filter out empty entries and keep last 4
            existing_runs = ['\n' + '=' * 70 + '\n' + run for run in runs if run.strip()][-4:]
        except OSError:
            pass

    # Write last 4 old runs + new run
    ...
    print(f"\nReports saved:")
    print(f"  {json_file}")
    print(f"  {backup_file} (previous run backup)")
    print(f"  {txt_file} (appended)")
...
def main():
    parser = argparse.ArgumentParser(description='Test Pine Script samples: transpile + pyne run')
    ...
    # Filter to only failed files if --retry-failed is set
    ...
    total_testable = len(testable_files)
    results = []
    ...
    runtime_ok = sum(1 for r in results if r['runtime'])
    return 0 if runtime_ok == total else 1
